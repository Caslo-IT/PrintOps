  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [filamentsData, printersData] = await Promise.all([
        api.getFilaments().catch(() => ({ filaments: [] })),
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h2 className="mt-3 text-3xl font-bold tracking-tight text-slate-900">Filament Inventory</h2>
          <p className="mt-1 text-sm text-slate-500">Manage filament spools and their assignments to printers.</p>
        </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div>
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">{filament.color} {filament.material}</p>
                  </div>
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleOpenModal(filament)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-md">
                      <Edit2 size={14} />
                    </button>
                    <button onClick={() => handleDelete(filament.id)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-md">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
                ...
                <div className="mt-6">
                  <div className="flex items-center justify-between text-sm mb-2">
                    ...
                    <span className={`font-medium ${isLow ? 'text-red-600' : 'text-slate-900'}`}>
                      {filament.remaining_weight_g.toFixed(1)}g / {filament.total_weight_g}g
                    </span>
                  </div>
        ...
        {filaments.length === 0 && !loading && (
          <div className="col-span-full py-12 text-center bg-white rounded-xl border border-slate-200 border-dashed">
            ...
            <p className="mt-1 text-sm text-slate-500">Get started by adding a new filament spool to track.</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <div className="mb-5 flex items-center justify-between">
              <h3 className="text-lg font-bold text-slate-900">{editingId ? 'Edit Filament' : 'Add Filament'}</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="mb-1 block text-sm font-medium text-slate-700">Total Weight (g)</label>
                  <input
                    type="number"
                    required
                    min="1"
                    ...
                    onChange={(e) => setFormData({ ...formData, total_weight_g: e.target.value })}
                  />
                </div>
                <div>
                  <label className="mb-1 block text-sm font-medium text-slate-700">Remaining (g)</label>
                  <input
                    type="number"
                    required
                    min="0"
              ...
              <div className="pt-4 flex justify-end gap-3">
                <button type="button" onClick={handleCloseModal} className="px-4 py-2 text-sm font-medium text-slate-700 hover:text-slate-900 hover:bg-slate-50 rounded-lg">
                  Cancel
                </button>
